...
void hmac_sha512(uchar *key, int key_length_bytes, uchar *message, int message_length_bytes, uchar *output) {
  ...
  sha512_bytes(inner_concat, 128+message_length_bytes, output);
  ...
  sha512_bytes(inner_concat, 192, output);
}
...
void sha256d(uchar *input, int input_len, uchar *output) {
  sha256_bytes(input, input_len, output);
  sha256_bytes(output, 32, output);
}

void hash160(uchar *input, int input_len, uchar *output) {
  uchar sha256_result[32] = { 0 };
  sha256_bytes(input, input_len, sha256_result);
